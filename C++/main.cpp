...
int main(int argc, char** argv)
{
   ...
   //line until only complete fingure contours are not taken, eliminating the palm by drawing line in the threshold output

   line( Handed_Thres, {0, Org_Frame.rows}, {Org_Frame.cols, Org_Frame.rows}, Scalar( 0, 0, 0 ), 380, LINE_8);

   medianBlur(Handed_Thres, Handed_Thres,blurSize);
        ...
        for( float j = 0; j < contours.size(); j++ )
           {
             ...
             Min_Rect[j].points(Rect_Points);
             for(int i = 0; i < 4; i++)
             line( Org_Frame, Rect_Points[i], Rect_Points[(i+1)%4], Scalar(0, 32, 255), 2, 8 );

           }

//        cv::putText(Org_Frame, //target image
//                    ("FPS: %d",), //text
